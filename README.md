...
一个面向实习项目展示的拼团交易系统。项目以 DDD 分层为主线，围绕「商品展示 -> 拼团试算 -> 锁单占位 -> 支付结算 -> 成团通知 -> 后台运营」构建完整业务闭环。

本仓库基于小傅哥 `group-buy-market` 学习项目继续扩展，重点补充了 C 端页面、后台管理、订单取消、Redis Lua 原子占位、结算通知、MQ 消费补偿、以及部分后台能力的领域化改造。
...
- **面试可讲的演进文档**：`docs/upgrade` 下整理了模块边界、表归属、API 归属和核心时序图。
...
1. 先讲业务闭环：拼团试算、锁单、结算、成团通知。
2. 再讲 DDD 分层：入口层、领域层、仓储端口、基础设施适配。
3. 展开一个技术点：Redis Lua 原子占位和失败补偿。
4. 展开一个可靠性点：`notify_task` 状态机和 MQ/HTTP 通知闭环。
5. 最后讲你做过的架构治理：controller 瘦身、隐藏副作用清理、接口归属文档。

## 后续优化方向

- 将后台订单、用户、活动管理继续抽成独立领域服务，进一步瘦身 `AdminController`。
- 用明确 DTO 替换后台接口里的 `Map<String, Object>`。
- 给通知任务增加更完整的死信/人工补偿视图。
- 为 Redis Lua 锁单补充更多并发场景测试。
- 把测试回调接口按 profile 隔离，避免生产环境误暴露。
